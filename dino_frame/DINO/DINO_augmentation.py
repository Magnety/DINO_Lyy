...
import numpy as np
import random
from dino_frame.utilities.to_torch import maybe_to_torch, to_cuda

from dino_frame.DINO.aug_utils import create_zero_centered_coordinate_mesh, elastic_deform_coordinates, \
    interpolate_img, \
...
class RandCrop_tu(nn.Module):
    ...
    def forward(self, x):
        ...
        for _b in range(b):
            d_seed = random.randint(0, x.shape[2]-self.crop_size[0])
            w_seed = random.randint(0, x.shape[3]-self.crop_size[1])
            h_seed = random.randint(0, x.shape[4]-self.crop_size[2])
            out[_b, :, :, :, :] = x[_b, :, d_seed:d_seed + self.crop_size[0], w_seed:w_seed + self.crop_size[1],
                                  h_seed:h_seed + self.crop_size[2]]
        return out
...
class Gaussiannoise_tu(nn.Module):
    ...
    def forward(self, x):
        ...
        for _b in range(b):
            ...
            signal_power = torch.norm(x[_b,:,:,:,:])**2/(c*d*w*h)
