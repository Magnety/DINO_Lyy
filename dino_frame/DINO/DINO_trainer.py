...
from dino_frame.network_architecture.neural_network import SegmentationNetwork
from dino_frame.network_architecture.neural_network import ClassficationNetwork
from dino_frame.loss_function.focal_loss import focal_loss
...
from dino_frame.evaluation.evaluator import aggregate_scores
from dino_frame.inference.segmentation_export import save_segmentation_nifti_from_softmax
from dino_frame.postprocessing.connected_components import determine_postprocessing
...
import matplotlib
from dino_frame.network_architecture.generic_VNet import VNet_class
from dino_frame.data_augmentation.dino_data_augmentation import default_3D_augmentation_params, default_2D_augmentation_params, get_default_augmentation, get_patch_size
from dino_frame.dataloading.dataset_load import load_dataset, DataLoader3D, unpack_dataset
from dino_frame.loss_function.Loss_functions import JI_and_Focal_loss, AutomaticWeightedLoss, softmax_helper, sum_tensor
from dino_frame.network_architecture.myVGG import vgg11_bn
...
class Trainer(nn.Module):
    ...
    def initialize(self, training=True):
        if not self.was_initialized:
            ...
            self.patch_size = np.array([64,192,192]).astype(int)
            ...
            if training:
                ...
                self.dino_loss = DINOLoss(
                        self.out_dim,
                        self.local_crops_number + 2,  # total number of crops = 2 global crops + local_crops_number
            ...
            self.lr_schedule =cosine_scheduler(
                self.initial_lr * (self.batch_size * get_world_size()) / 256.,  # linear scaling rule
                self.lr_threshold,
                self.epochs, self.num_batches_per_epoch//self.batch_size,
            ...
            self.momentum_schedule = cosine_scheduler(self.momentum_teacher, 1,
                                                       self.epochs, self.num_batches_per_epoch//self.batch_size,)
            params_groups = self.student_network.parameters()
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if not isinstance(global_data1, torch.Tensor):
            global_data1 = torch.from_numpy(global_data1).float()
        if not isinstance(global_data2, torch.Tensor):
            global_data2 = torch.from_numpy(global_data2).float()
        if not isinstance(local_data, torch.Tensor):
            local_data = torch.from_numpy(local_data).float()
        if not isinstance(class_target, torch.Tensor):
            class_target = torch.from_numpy(class_target).float()
        data = data.cuda(non_blocking=True)
        global_data1 = global_data1.cuda(non_blocking=True)
        global_data2 = global_data2.cuda(non_blocking=True)
        local_data = local_data.cuda(non_blocking=True)
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        save_dict = {
            'student': self.student_network.state_dict(),
            'teacher': self.teacher_network.state_dict(),
    ...
    def run_trainer(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            for b in range(self.num_batches_per_epoch):
                self.it = ((self.num_batches_per_epoch//self.batch_size)*self.epoch+b)
