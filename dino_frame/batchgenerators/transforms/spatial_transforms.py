...
from dino_frame.batchgenerators.transforms.abstract_transforms import AbstractTransform
from dino_frame.batchgenerators.augmentations.spatial_transformations import augment_spatial, augment_spatial_2, \
    augment_channel_translation, \
    augment_mirroring, augment_transpose_axes, augment_zoom, augment_resize, augment_rot90,dino_augment_spatial
import numpy as np
...
class ChannelTranslation(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        return data_dict
class dino_SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size,global_patch_size,local_patch_size, patch_center_dist_from_border=30,
                 ...
                 label_key="seg",global_data1_key="global_data1", global_data2_key="global_data2",local_data_key="local_data",p_el_per_sample=1, p_scale_per_sample=1, p_rot_per_sample=1,
        ...
        self.global_data1_key = global_data1_key
        self.global_data2_key = global_data2_key
        self.local_data_key = local_data_key
    ...
    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        seg = data_dict.get(self.label_key)
        global_data1 = data_dict.get(self.global_data1_key)
        global_data2 = data_dict.get(self.global_data2_key)
        local_data = data_dict.get(self.local_data_key)
        ...
        for _ in range(local_data.shape[0]):
            local_ret_val = dino_augment_spatial(local_data[_], None, patch_size=self.local_patch_size,
                                                 ...
                                                 p_rot_per_axis=self.p_rot_per_axis,
                                                 p_independent_scale_per_axis=self.p_independent_scale_per_axis)
            _local.append(local_ret_val[0][None])
        _local = np.vstack(_local)


        ret_val = dino_augment_spatial(data, seg, patch_size=patch_size,
                                  ...
                                  p_independent_scale_per_axis=self.p_independent_scale_per_axis)
        data_dict[self.data_key] = ret_val[0]
        data_dict[self.global_data1_key] = global_ret_val1[0]
        data_dict[self.global_data2_key] = global_ret_val2[0]
        data_dict[self.local_data_key] = _local
