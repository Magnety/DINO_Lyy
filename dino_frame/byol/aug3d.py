...
from dino_frame.byol.utils import create_zero_centered_coordinate_mesh, elastic_deform_coordinates, \
    interpolate_img, \
...
class RandCrop_tu(nn.Module):
    ...
    def forward(self, x):
        ...
        for _b in range(b):
            d_seed = random.randint(20, 54)
            w_seed = random.randint(20, 54)
            h_seed = random.randint(20, 54)
            out[_b,:,:,:,:] = x[_b,:,d_seed:d_seed+self.crop_size[0],w_seed:w_seed+self.crop_size[1],h_seed:h_seed+self.crop_size[2]]
        return out
