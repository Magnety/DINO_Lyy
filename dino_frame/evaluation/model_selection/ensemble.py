...
from dino_frame.paths import network_training_output_dir, preprocessing_output_dir
...
def ensemble(training_output_folder1, training_output_folder2, output_folder, task, validation_folder, folds, allow_ensembling: bool = True):
    ...
    for f in folds:
        ...
        if not isdir(validation_folder_net1):
            raise AssertionError("Validation directory missing: %s. Please rerun validation with `simple_frame_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net1)
        if not isdir(validation_folder_net2):
            raise AssertionError("Validation directory missing: %s. Please rerun validation with `simple_frame_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net2)
        ...
        if not isfile( validation_folder_net1+"/"+ 'summary.json' ):
            raise AssertionError("Validation directory incomplete: %s. Please rerun validation with `simple_frame_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net1)
