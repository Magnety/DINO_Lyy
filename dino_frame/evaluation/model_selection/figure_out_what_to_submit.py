...
from dino_frame.evaluation.add_mean_dice_to_json import foreground_mean
from dino_frame.evaluation.evaluator import evaluate_folder
from dino_frame.evaluation.model_selection.ensemble import ensemble
from dino_frame.paths import network_training_output_dir
import numpy as np
from subprocess import call
from dino_frame.postprocessing.consolidate_postprocessing import consolidate_folds, collect_cv_niftis
from dino_frame.utilities.folder_names import get_output_folder_name
from dino_frame.paths import default_cascade_trainer, default_trainer, default_plans_identifier
...
def main():
    ...
    parser.add_argument("-tr", type=str, required=False, default=default_trainer,
                           help="simple_frameTrainer class. Default: %s" % default_trainer)
    parser.add_argument("-ctr", type=str, required=False, default=default_cascade_trainer,
                           help="simple_frameTrainer class for cascade model. Default: %s" % default_cascade_trainer)
        ...
        for k, v in results.items():
            if v == best:
                ...
                if k.startswith("ensemble"):
                    ...
                    predict_str += "simple_frame_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + m1 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                    predict_str += "simple_frame_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr " + tr + " -ctr " + trc + " -m " + m2 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"

                    if not disable_postprocessing:
                        predict_str += "simple_frame_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER -pp " +  network_training_output_dir+"/"+"ensembles"+"/"+ id_task_mapping[t] +"/"+ k+"/"+ "postprocessing.json"  + "\n"
                    else:
                        predict_str += "simple_frame_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER\n"
                else:
                    predict_str += "simple_frame_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + k + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                print(predict_str)
