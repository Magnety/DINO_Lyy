...
import os
from dino_frame.evaluation.model_selection.summarize_results_in_one_json import summarize
...
if __name__ == "__main__":
    ...
    simple_frames = ['simple_frameV2', 'simple_frameV2_zspacing']
    ...
    with open("summary.csv", 'w') as f:
        ...
        for i in task_ids:
            for simple_frame in simple_frames:
                try:
                    summary_folder =  base_dir+"/"+ simple_frame+"/"+ "summary_fold0"
                    if isdir(summary_folder):
                        ...
                        for s in summary_files:
                            ...
                            expected_output_folder =  base_dir+"/"+ simple_frame+"/"+ tmp[1]+"/"+ tmp[0]+"/"+tmp[2].split(".")[0]
                            name = tmp[0] + "__" + simple_frame + "__" + tmp[1] + "__" + tmp[2].split(".")[0]
                            global_dice_json =  base_dir+"/"+ simple_frame+"/"+ tmp[1]+"/"+ tmp[0]+"/"+ tmp[2].split(".")[0]+"/"+ "fold_0"+"/"+ "validation_tiledTrue_doMirror_True"+"/"+ "global_dice.json"

                            if not isdir(expected_output_folder) or len(tmp) > 3:
                                if len(tmp) == 2:
                                    continue
                                expected_output_folder =  base_dir+"/"+ simple_frame+"/"+ tmp[1]+"/"+ tmp[0]+"/"+ tmp[2] + "__" + tmp[3].split(".")[0]
                                name = tmp[0] + "__" + simple_frame + "__" + tmp[1] + "__" + tmp[2] + "__" + tmp[3].split(".")[0]
                                global_dice_json =  base_dir+"/"+ simple_frame+"/"+ tmp[1]+"/"+ tmp[0]+"/"+ tmp[2] + "__" + tmp[3].split(".")[0]+"/"+"fold_0"+"/"+ "validation_tiledTrue_doMirror_True"+"/"+ "global_dice.json"
