...
from dino_frame.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from dino_frame.experiment_planning.utils import crop
from dino_frame.paths import *
import shutil
from dino_frame.utilities.task_name_id_conversion import convert_id_to_task_name
from dino_frame.preprocessing.sanity_checks import verify_dataset_integrity
from dino_frame.training.model_restore import recursive_find_python_class
...
def main():
    ...
    for i in task_ids:
        ...
        if args.verify_dataset_integrity:
            verify_dataset_integrity(raw_data+"/"+task_name )
        crop(task_name, False, tf)
        tasks.append(task_name)


    search_in = dino_frame.__path__[0] + "/" + "experiment_planning"
    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="dino_frame.experiment_planning")
        if planner_3d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "dino_frame.experiment_planning" % planner_name3d)
    else:
        planner_3d = None

    """if planner_name2d is not None:
        planner_2d = recursive_find_python_class([search_in], planner_name2d, current_module="dino_frame.experiment_planning")
        if planner_2d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "dino_frame.experiment_planning" % planner_name2d)
    else:
        planner_2d = None"""

    for t in tasks:
        print("\n\n\n", t)
        cropped_out_dir =cropped_data+"/"+ t
        preprocessing_output_dir_this_task =  preprocessing_output_dir+"/"+ t
        #splitted_4d_output_dir_task = os.path.join(simple_frame_raw_data, t)
        ...
        shutil.copy( raw_data+"/"+ t +"/"+ "dataset.json" , preprocessing_output_dir_this_task)
