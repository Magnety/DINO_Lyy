...
import shutil
from dino_frame.paths import default_cascade_trainer, default_plans_identifier, default_trainer, network_training_output_dir
from dino_frame.utilities.task_name_id_conversion import convert_id_to_task_name
...
def copy_pretrained_models_for_task(task_name: str, output_directory: str,
                                    ...
                                    simple_frame_trainer=default_trainer,
                                    simple_frame_trainer_cascade=default_cascade_trainer,
                                    plans_identifier=default_plans_identifier):
    trainer_output_dir = simple_frame_trainer + "__" + plans_identifier
    trainer_output_dir_cascade = simple_frame_trainer_cascade + "__" + plans_identifier
...
def export_one_task(taskname, models, output_folder, simple_frame_trainer=default_trainer,
                    simple_frame_trainer_cascade=default_cascade_trainer,
                    plans_identifier=default_plans_identifier):
    copy_pretrained_models_for_task(taskname, output_folder, models, simple_frame_trainer, simple_frame_trainer_cascade,
                                    plans_identifier)
    copy_ensembles(taskname, output_folder, models, (simple_frame_trainer, simple_frame_trainer_cascade), (plans_identifier,))
...
def export_pretrained_model(task_name: str, output_file: str,
    ...
    for m in models:
        ...
        if not isfile( expected_output_folder+"/"+ "postprocessing.json") :
            if strict:
                raise RuntimeError('postprocessing.json missing. Run simple_frame_determine_postprocessing or disable strict')
            else:
    ...
    valid = []
    for s in subd:
        v = check_if_valid(s, models, (simple_frame_trainer, simple_frame_trainer_cascade), (plans_identifier))
        if v:
            valid.append(s)
    for v in valid:
...
def export_entry_point():
    ...
    parser = argparse.ArgumentParser(description="Use this script to export models to a zip file for sharing with "
                                                 ...
                                                 "for usage with simple_frame_download_pretrained_model_by_url, or share the "
                                                 "zip for usage with simple_frame_install_pretrained_model_from_zip")
    ...
    parser.add_argument('-pl', type=str, help='dino_frame plans identifier. Default: %s' % default_plans_identifier,
...
def export_for_paper():
    output_base = "/media/fabian/DeepLearningData/simple_frame_trained_models"
