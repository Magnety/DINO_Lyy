...
class VNet_recons(SegmentationNetwork):
    ...
    def __init__(self, num_classes,deep_supervision,elu=True, nll=False):
        ...
        self.down_tr64 = DownTransition(2*channels, 2, elu, (1,2,2)) #64
        self.down_tr128 = DownTransition(4*channels, 3, elu, (1,2,2), dropout=True) #128
        ...
        self.out_tr = OutputTransition(2*channels,self.num_classes, elu, nll)
...
class VNet_class(ClassficationNetwork):
    # the number of convolutions in each layer corresponds
    # to what is in the actual prototxt, not the intent
    def __init__(self, num_classes,deep_supervision,elu=True, nll=False):
        super(VNet_class, self).__init__()
        self.do_ds = False
        norm_cfg = 'BN'
        activation_cfg = 'ReLU'
        self.conv_op = nn.Conv3d
