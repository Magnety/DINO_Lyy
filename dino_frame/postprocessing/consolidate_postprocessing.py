...
def collect_cv_niftis(cv_folder: str, output_folder: str, validation_folder_name: str = 'validation_raw',
    ...
    validation_raw_folders = [ cv_folder+"/"+ "fold_%d" % i+"/"+ validation_folder_name  for i in folds]
...
def consolidate_folds(output_folder_base, validation_folder_name: str = 'validation_raw',
    ...
    num_niftis_gt = len(subfiles(output_folder_base+"/"+ "gt_niftis" , suffix='.nii.gz'))
    ...
    summary_fold0 = load_json(  output_folder_base+"/"+ "fold_0"+"/"+ validation_folder_name+"/"+ "summary.json"  )['results'][
        'mean']
    classes = [int(i) for i in summary_fold0.keys()]
    niftis = subfiles(output_folder_raw, join=False, suffix=".nii.gz")
    test_pred_pairs = [( output_folder_gt+"/"+ i ,  output_folder_raw+"/"+ i ) for i in niftis]
    ...
    aggregate_scores(test_pred_pairs, labels=classes, json_output_file= output_folder_raw+"/"+ "summary.json" ,
                     num_threads=4)
