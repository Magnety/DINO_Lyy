...
import importlib
import pkgutil
from dino_frame.training.cls_trainer import Trainer
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    """
    This is a utility function to load any dino_frame trainer from a pkl. It will recursively search
    dino_frame.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    is specified, it will furthermore load the checkpoint file in train/test mode (as specified by train).
    The pkl file required here is the one that will be saved automatically when calling simple_frameTrainer.save_checkpoint.
    :param pkl_file:
    :param checkpoint:
    :param train:
    ...
    init = info['init']
    name = info['name']
    search_in = dino_frame.__path__[0] + "/" + "training" + "/" + "network_training"
    tr = recursive_find_python_class([search_in], name, current_module="dino_frame.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in dino_frame.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within dino_frame.trainig.network_training (search is recursive)."
                           "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s " % (checkpoint, name))
    assert issubclass(tr, Trainer), "The network trainer was found but is not a subclass of simple_frameTrainer. " \
                                          "Please make it so!"

    # this is now deprecated
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of dino_frame. Attempting to load it "
...
if __name__ == "__main__":
    pkl = "/home/fabian/PhD/results/simple_frameV2/simple_frameV2_3D_fullres/Task004_Hippocampus/fold0/model_best.model.pkl"
    checkpoint = pkl[:-4]
    train = False
