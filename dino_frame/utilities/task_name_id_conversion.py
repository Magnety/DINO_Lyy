...
from dino_frame.paths import raw_data, preprocessing_output_dir, cropped_data, network_training_output_dir
...
def convert_id_to_task_name(task_id: int):
    ...
    if raw_data is not None:
        candidates_raw = subdirs(raw_data, prefix=startswith, join=False)
    else:
        candidates_raw = []

    if cropped_data is not None:
        candidates_cropped = subdirs(cropped_data, prefix=startswith, join=False)
    else:
        candidates_cropped = []
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError("More than one task name found for task id %d. Please correct that. (I looked in the "
                           "following folders:\n%s\n%s\n%s" % (task_id, raw_data, preprocessing_output_dir,
                                                               cropped_data))
    if len(unique_candidates) == 0:
        raise RuntimeError("Could not find a task with the ID %d. Make sure the requested task ID exists and that "
                           ...
                           "Installation). Here are your currently defined folders:\nsimple_frame_preprocessed=%s\nRESULTS_"
                           "FOLDER=%s\nsimple_frame_raw_data_base=%s\nIf something is not right, adapt your environemnt "
                           "variables." %
                           (task_id,
                            os.environ.get('simple_frame_preprocessed') if os.environ.get('simple_frame_preprocessed') is not None else 'None',
                            os.environ.get('RESULTS_FOLDER') if os.environ.get('RESULTS_FOLDER') is not None else 'None',
                            os.environ.get('simple_frame_raw_data_base') if os.environ.get('simple_frame_raw_data_base') is not None else 'None',
                            ))
